...
#              endif
               ...
               /* Send data on both sides of the dividing line from rank 0 to rank 1 */
#              ifdef MPI
               int Tag = itr;

               if ( MyRank == 0 )
               {
                  MPI_Send( Potential[Nx/2-1], Ny, MPI_DOUBLE, 1, Tag,   MPI_COMM_WORLD );
                  MPI_Recv( Potential[Nx/2]  , Ny, MPI_DOUBLE, 1, Tag+1, MPI_COMM_WORLD, MPI_STATUSES_IGNORE );
               }
               else if ( MyRank == 1 )
               {
                  MPI_Recv( Potential[0]     , Ny, MPI_DOUBLE, 0, Tag,   MPI_COMM_WORLD, MPI_STATUSES_IGNORE );
                  MPI_Send( Potential[1]     , Ny, MPI_DOUBLE, 0, Tag+1, MPI_COMM_WORLD );
               }
#              endif
...
#              endif
               ...
               /* Send data on both sides of the dividing line from rank 1 to rank 0 */
#              ifdef MPI
               Tag =Tag+10;
...
/* allocate memory to store potential */
# ifdef MPI
...
/* copy data from Potential on rank1 to Potential_All on rank0 */
  if ( MyRank == 1 )
  {
    MPI_Send( &Potential[0][0],        (Nx/2)*Ny, MPI_DOUBLE, 0, 123, MPI_COMM_WORLD );
    MPI_Send( &Mass     [0][0],        (Nx/2)*Ny, MPI_DOUBLE, 0, 124, MPI_COMM_WORLD );
  }

  if ( MyRank == 0 )
  {
    MPI_Recv( &Potential_All[Nx/2][0], (Nx/2)*Ny, MPI_DOUBLE, 1, 123, MPI_COMM_WORLD, MPI_STATUSES_IGNORE );
    MPI_Recv( &Mass_All     [Nx/2][0], (Nx/2)*Ny, MPI_DOUBLE, 1, 124, MPI_COMM_WORLD, MPI_STATUSES_IGNORE );
  }
